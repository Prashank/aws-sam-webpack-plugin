  ...
  public initProject(projectKey: string): void {
    ...
    for (const resourceKey in templateYml.Resources) {
      ...
      if (Type === "AWS::Serverless::Function") {
        ...
        if (!["nodejs10.x", "nodejs12.x", "nodejs14.x", "nodejs16.x"].includes(properties.Runtime ?? defaultRuntime)) {
          throw new Error(`${resourceKey} has an unsupport Runtime. Must be nodejs10.x, nodejs12.x or nodejs14.x or "nodejs16.x"`);
        }
